...
export const getEntityEngagement = async (req: ICustomRequest, res: Response) => {
  ...
  const data = await entityEngagementService.getStats(entityType, entityId, req.user?.id);
  return res.status(200).json({ data });
};

export const getEntityRatings = async (req: ICustomRequest, res: Response) => {
  const entityType = asString(req.params.entityType);
  const entityId = asString(req.params.entityId);
...
export const deleteEntityRating = async (req: ICustomRequest, res: Response) => {
  const entityType = asString(req.params.entityType);
  const entityId = asString(req.params.entityId);
  ...
  const data = await entityEngagementService.deleteRating(entityType, entityId, req.user.id);
  return res.status(200).json({ data });
};
