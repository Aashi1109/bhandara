...
describe('cache key compaction', () => {
  it('builds compact engagement keys', () => {
    ...
    expect(cacheKeys.engagementViewDedupe('messages', 'msg-1', '2026-03-23', 'viewer-1')).toBe(
      'egv:m:msg-1:2026-03-23:viewer-1',
    );
  });
  ...
  it('obscures sensitive lookup keys', () => {
    const emailKey = cacheKeys.userEmailLookup('user@example.com');
    const usernameKey = cacheKeys.userUsernameLookup('tester');
    const otpKey = cacheKeys.passwordResetOtp('user@example.com');
    const tokenKey = cacheKeys.passwordResetToken('reset-token');

    expect(emailKey).not.toContain('user@example.com');
    expect(usernameKey).not.toContain('tester');
    expect(otpKey).not.toContain('user@example.com');
    expect(tokenKey).not.toContain('reset-token');
    expect(emailKey.length).toBeLessThan(usernameKey.length);

    expect(cacheKeys.userEmailLookup('user@example.com')).toBe(emailKey);
    expect(cacheKeys.passwordResetToken('reset-token')).toBe(tokenKey);
  });
});
